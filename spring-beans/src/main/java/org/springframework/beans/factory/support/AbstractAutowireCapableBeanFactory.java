...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
    ...
    protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
        ...
        // 锁定class，根据设置的class属性或者根据className来解析class,得到beanName对应的Class对象
    ...
    protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
        ...
        // Allow post-processors to modify the merged bean definition.
		 ...
		 实例化完成后为属性填充和和初始化做准备，比如解析AutoWired,Value,Resource,PostConstruct,PreDestroy等注解，
		 解析后分别缓存起来
		关键类InitDestroyAnnotationBeanPostProcessor，AutowiredAnnotationBeanPostProcessor（AutoWired,Value），
...
//			registeredSingletons.add(beanName);
    ...
    /*
        主要有四种属性填充的方式：
        1.调用postProcessAfterInstantiation完成属性赋值，可以直接终止后面的处理工作，也可以让后续的步骤完成覆盖操作
        2.根据配置文件autoWired属性来决定使用名称还是类型注入 autowireByName(),autowireByType()
        3.根据对象中定义的@AutoWired注解解析，完成对象或者属性的注入 postProcessProperties()--->AutowiredAnnotationBeanPostProcessor
        4.根据property标签定义的属性值，完成对应的解析和赋值工作 applyPropertyValues()
     */
    ...
    protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
        ...
        for (PropertyValue pv : original) {
            ...
            // 如果属性没有被解析过
                ...
                //如果可转换
                if (convertible) {
                    //将resolvedValue转换为指定的目标属性对象，里面会调用用户的转换器和属性编辑器
